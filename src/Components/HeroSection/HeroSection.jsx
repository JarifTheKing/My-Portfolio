...
export default function HeroSection() {
  ...
  const socialLinks = [
    { icon: <FaFacebookF />, href: "https://www.facebook.com/ddewan.jjamil" },
    { icon: <FaLinkedinIn />, href: "https://www.linkedin.com/in/mahfuz-billah/" },
    { icon: <FaGithub />, href: "https://github.com/JarifTheKing" },
    { icon: <FaWhatsapp />, href: "https://wa.me/8801904947118" },
    { icon: <BiLogoGmail />, href: "mailto:mahfuzbillah24@gmail.com" },
  ];

  return (
    <section
      id="home"
      className="relative w-full min-h-screen flex items-center px-6 md:px-16 py-10 md:py-20 overflow-hidden rounded-2xl transition-colors duration-500 bg-slate-50 dark:bg-slate-950"
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-white/80 via-slate-50/50 to-white/80 dark:from-gray-900 dark:via-gray-900/90 dark:to-gray-800 transition-colors duration-500" />
      ...
      <div className="relative z-10 container mx-auto flex flex-col-reverse md:flex-row items-center justify-between gap-10 md:gap-20">
        ...
        <motion.div
          ...
          variants={fadeUp}
        >
          <p className="inline-flex items-center gap-2 bg-green-500/10 dark:bg-green-500/20 text-green-600 dark:text-green-400 font-medium mb-4 px-4 py-1 rounded-full mx-auto md:mx-0 backdrop-blur-md border border-green-500/30">
            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
            Available for work
          </p>

          <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white leading-tight transition-colors duration-500">
            Junior Frontend{" "}
          ...
          <p className="text-slate-600 dark:text-gray-300 mt-4 max-w-md text-lg mx-auto md:mx-0 leading-relaxed transition-colors duration-500">
            Skilled in React.js and modern JavaScript, passionate about building
            clean, responsive, and user-friendly web interfaces.
          </p>
          ...
          <div className="flex flex-wrap gap-6 mt-6 justify-center md:justify-start">
            {[
              ...
              <motion.div
                ...
                transition={{ duration: 2, repeat: Infinity, delay: idx * 0.3 }}
              >
                <p className="text-2xl font-bold text-indigo-600 dark:text-blue-500">{stat.num}</p>
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-8 justify-center md:justify-start">
            <motion.a
              href="#projects"
              className="px-6 py-3 bg-indigo-600 dark:bg-gradient-to-r dark:from-purple-700 dark:to-purple-900 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 text-center"
              whileHover={{ scale: 1.05 }}
            >
              View My Work →
            </motion.a>
            ...
            <motion.a
              href="#"
              target="_blank"
              className="group relative inline-block p-[2px] rounded-lg bg-slate-200 dark:bg-purple-900/50 hover:bg-indigo-500 transition-all duration-300"
              whileHover={{ scale: 1.05 }}
            >
              <div className="bg-white dark:bg-gray-800 rounded-lg px-6 py-3 text-indigo-600 dark:text-purple-400 font-semibold group-hover:bg-transparent group-hover:text-white transition-all">
                Download Resume
              </div>
            </motion.a>
          </div>
          ...
          <div className="flex gap-4 justify-center md:justify-start mt-8">
            {socialLinks.map((link, idx) => (
              <a
                key={idx}
                href={link.href}
                className="p-3 text-slate-600 dark:text-gray-300 bg-white dark:bg-gray-800/20 border border-slate-200 dark:border-purple-700 rounded-full hover:bg-indigo-600 dark:hover:bg-purple-700 hover:text-white transition-all duration-300 shadow-sm"
              >
                {link.icon}
              </a>
            ))}
        ...
        <div className="relative mb-10 md:mb-0 flex justify-center md:justify-end">
          <motion.div
            className="relative w-64 h-64 md:w-80 md:h-80 rounded-full p-2 bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 shadow-2xl"
            animate={{ y: [0, -10, 0] }}
          ...
          {[
            { icon: <FaReact />, label: "React", pos: "top-0 left-0", d: 0 },
            { icon: <TbBrandNextjs />, label: "Next.js", pos: "bottom-0 left-0", d: 0.5 },
            { icon: <FaBolt />, label: "Fast", pos: "bottom-0 right-0", d: 1 },
            { icon: <FaPaintBrush />, label: "Design", pos: "top-1/2 -left-12", d: 1.5 },
          ].map((badge, idx) => (
            <motion.div
              key={idx}
              className={`absolute ${badge.pos} bg-white dark:bg-purple-700 text-indigo-600 dark:text-white px-3 py-1 rounded-full flex items-center gap-1 shadow-lg text-xs md:text-sm font-bold border border-indigo-100 dark:border-none`}
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 2, repeat: Infinity, delay: badge.d, ease: "easeInOut" }}
            >
              {badge.icon} {badge.label}
            </motion.div>
          ))}
